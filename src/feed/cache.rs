...
impl FeedCache {
    ...
    pub fn update_feed(
        ...
        let cached = self.feeds.entry(url.to_string()).or_insert_with(|| CachedFeed {
            url: url.to_string(),
            name: name.to_string(),
            items: Vec::new(),
            last_fetched: None,
        ...
        if cached.last_error.is_none() {
            ...
            let old_states: HashMap<String, bool> = cached.items
                .iter()
                .map(|i| (i.id.clone(), i.read))
                .collect();
            
            cached.items = items.into_iter().map(|mut item| {
    ...
    pub fn stats(&self) -> CacheStats {
        ...
        let unread_items: usize = self.feeds.values()
            .flat_map(|f| f.items.iter())
            .filter(|i| !i.read)
            .count();
        
        let oldest_fetch = self.feeds.values()
            .filter_map(|f| f.last_fetched)
            .min();
        ...
        for feed in self.feeds.values_mut() {
            if feed.items.len() > max_items_per_feed {
                ...
                if feed.items.len() < old_len {
                    self.dirty = true;
                    debug!("Pruned {} items from {}", old_len - feed.items.len(), feed.name);
                }
            }
        }
    }
